	...
	if($(".box1").length>0){
		$(".box1").each(function(){
			...
			if($(this).attr("panelHeight")!=null){
				...
				var panelHeight=$(this).attr("panelHeight");
				
				var lastStr=panelHeight.substr(panelHeight.length-1,1);
				
				if(lastStr=="%"){
				
					var maiheight = document.documentElement.clientHeight;
					var h=Number(panelHeight.substr(0,panelHeight.length-1))/100;
					
					$(this).find(".box1_topcenter").height(20);
					$(this).find(".box1_bottomcenter").height(22);
					
					var useHeight=(maiheight*h)-$(this).find(".box1_topcenter").outerHeight()-$(this).find(".box1_bottomcenter").outerHeight();
...
jQuery.SelectBox = function(selectobj, options) {
	...
	}else if(broswerFlag=="Opera"){
		
	}else if(broswerFlag=="Safari"){
		
	}else if(broswerFlag=="Other"){
	...
	function loadLater(obj,value){
					...
					var dataPath;
					if (obj.attr("childDataType") == null||obj.attr("childDataType")=="undefined") {
						dataPath = obj.attr("childDataPath");
					}
					else 
...
function showProgressBar(str){
	...
	diag.Height = 70;
	diag.Title = titleStr;
	diag.InnerHtml = "<html xmlns='http://www.w3.org/1999/xhtml'>" +
			"<head><meta http-equiv='Content-Type' content='text/html; charset=utf-8' />" +
			"</head><body>" +
			"<div style='margin:auto'><img src='"+window.parent.path+"/bin/icons/loading_progress.gif' /></div>" +
			"</body>" +
			"</html>";
