		...
		validateCall: function(caller, rules) {
			...
			for (i = 0; i < rules.length; i++) {
				...
				case "confirm":
					...
					break;
				case "bothEmpty":
					_bothEmpty(caller, rules, i);
					break;
				case "reg_isCard":
					_reg_isCard(caller, rules, i);
					break;
				case "reg_isFloat":
					_reg_isFloat(caller, rules, i);
					break;
				case "compareDate":
					_compareDate(caller, rules, i);
					break;
				case "funcCall":
			...
			function _bothEmpty(caller, rules, position) {
				if ($(caller).attr("value") == "") {
					otherField = rules[position + 1];
					if ($(caller).attr("value") == "" && $("#" + otherField).attr("value") == "" ) {
						firstField = eval(rules[position + 2]);
						secondField = eval(rules[position + 3]);
						$.validationEngine.isError = true;
						promptText += $.validationEngine.settings.allrules.bothEmpty.alertText1 + firstField + $.validationEngine.settings.allrules.bothEmpty.alertText2 + secondField + $.validationEngine.settings.allrules.bothEmpty.alertText3 + "<br />"
					}
				}
			}
			function _reg_isCard(caller, rules, position) {
				pattern = /^(\d{15}|\d{17}([0-9xX]))$/;
				if ($(caller).attr("value") != "") {
					if (!pattern.test($(caller).attr("value"))) {
						$.validationEngine.isError = true;
						promptText += $.validationEngine.settings.allrules.reg_isCard.alertText + "<br />"
					}
				}
			}
			function _reg_isFloat(caller, rules, position) {
				pattern = /^(-?\d+)(\.\d+)?/;
				if ($(caller).attr("value") != "") {
					if ( isNaN($(caller).attr("value")) || !pattern.test($(caller).attr("value"))) {
						$.validationEngine.isError = true;
						promptText += $.validationEngine.settings.allrules.reg_isFloat.alertText + "<br />"
					}
				}
			}
			function _compareDate(caller, rules, position) {
				if ($(caller).attr("value") != "") {
					var c = rules[position + 1];
					var d1 = rules[position + 2];
					var flag =true;
					var txt, t1, t2, dd;
					if ($("#"+d1) && $("#"+d1).attr("value") != "" ) {
						t1 = $(caller).attr("error")?$(caller).attr("error"):"截止日期";
						t2 = $("#"+d1).attr("error")?$("#"+d1).attr("error"):"起始日期";
						try{
							d1 = $("#"+d1).attr("value").replace(/-/g,"/");
							dd = $(caller).attr("value").replace(/-/g,"/");
							d1 = new Date(d1);
							dd = new Date(dd);
						}catch(e){
				           $.validationEngine.isError = true;
						...
						switch(c){
					 		case ">":
					 			txt ="要大于";
					 			if(Date.parse(dd)>Date.parse(d1))flag = false;
					   			break;
					   		case "<":
					 			txt ="要小于";
					 			if(Date.parse(dd)<Date.parse(d1))flag = false;
						...
						$.validationEngine.isError = flag;
						promptText += $.validationEngine.settings.allrules.compareDate.alertText + t1 + txt + t2+ "<br />"
					}
				}
			}
