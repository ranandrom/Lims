	...
	if(settings.inlineValidation == true){ 		// Validating Inline ?
		if(!settings.returnIsValid){					// NEEDED FOR THE SETTING returnIsValid
			...
			if(settings.liveEvent){						// LIVE event, vast performance improvement over BIND
				...
				$(this).find("select[class*=validate]").live("close", function(caller){_inlinEvent(this); })
				$(this).find("input:text[class*=date]").live("blur", function(caller){_inlinEvent(this);})
				$(this).find("[class*=selectTree]").live("close", function(caller){_inlinEvent(this);})
				...
				$(this).find("select[class*=validate]").bind("close", function(caller){_inlinEvent(this); })
				$(this).find("input:text[class*=date]").bind("blur", function(caller){_inlinEvent(this); })
				$(this).find("[class*=selectTree]").bind("close", function(caller){_inlinEvent(this);})
			}
			firstvalid = false;
...
$.validationEngine = {
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		function _required(caller,rules){   // 必填处理
			if($(caller).attr("class")=="selectTree validate[required]"){
				//alert($(caller).attr("relValue"))
	...
	buildPrompt : function(caller,promptText,type,ajaxed) {			// 出现提示框
		if(!$.validationEngine.settings){
			$.validationEngine.defaultSetting()
		}
