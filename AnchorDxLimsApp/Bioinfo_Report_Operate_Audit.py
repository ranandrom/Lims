...
# 运营审核报告任务列表
def Bioinfo_Report_Operate_Audit_Task_Review(request):
    try:
        ...
        temp_unfinished = models.BioinfoDataAnalysisInfo.objects.filter(Report_Make_Sign=1, Medical_Examine_Result='通过',
                                                                        Operate_Examine_Sign=0)
        temp_finished = models.BioinfoDataAnalysisInfo.objects.filter(Report_Make_Sign=1, Medical_Examine_Result='通过',
                                                                      Operate_Examine_Sign=1)

        return render(request, "modelspage/Bioinfo_Report_Operate_Audit_Task_Review.html",
...
# 运营审核报告信息录入页
def Bioinfo_Report_Operate_Audit_Info_Input (request):
    try:
        username = request.session['username']
        department = request.session['department']
    except Exception:
        ...
        return render(request, "modelspage/Bioinfo_Report_Operate_Audit_Submit.html",
...
#  运营审核报告信息录入到数据库以及报告文件下载
def Bioinfo_Report_Operate_Audit_InfoToDataBases(request):
    button_name = ''  # 按钮名字
    ...
    # 运营审核报告信息
    Operate_Examine_Result = ''  # 审核结果
    Operate_Examine_Time = ''  # 审核时间
    Operate_Examine_Remarks = ''  # 审核备注
    Report_Send_Man = ''  # 报告发送人
    NoPass = ''  # 不通过
    ...
    if request.method == "POST":
        ...
        # 审核结果
        Operate_Examine_Result = request.POST.get('Operate_Examine_Result')
        print '审核结果: ', Operate_Examine_Result

        # 审核时间
        Operate_Examine_Time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print '审核时间: ', Operate_Examine_Time

        # 审核备注
        Operate_Examine_Remarks = request.POST.get('Operate_Examine_Remarks')
        print '审核备注: ', Operate_Examine_Remarks

        Report_Send_Man = request.POST.get('Report_Send_Man')  # 运营审核人
    ...
    try:
        ...
        if button_name == 'download':
            ...
            if temp_UserOperationPermissionsInfo.BioinfoReportOperateAuditTaskReview == '1':
                # 添加数据到数据库
                models.BioinfoDataAnalysisInfo.objects.filter(sam_code_num=sam_code_num,
                                                              DataAnalysis_num=DataAnalysis_num).update(
                    Operate_Examine_Result=Operate_Examine_Result,
                    Operate_Examine_Time=Operate_Examine_Time,
                    Operate_Examine_Remarks=Operate_Examine_Remarks,
                    Operate_Examine_Sign='1',
                    Report_Send_Man=Report_Send_Man,
                )

                if Operate_Examine_Result == '通过':
                    user = Report_Send_Man
                    # 添加系统消息
                    Title = '通知：生信报告发送任务'  # 系统消息标题
                    Message = username + '分派给你一个生信报告发送任务！样本编号为：' + sam_code_num + '。请尽快完成任务！'  # 系统邮件正文
                else:
                ...
                models.UserSystemMessage.objects.create(
                    ...
                    Receiver=user,  # 接收者
                    # 信息内容
                    Time=Operate_Examine_Time,  # 信息生成时间
                    Title=Title,  # 系统消息标题
            ...
            if temp_UserOperationPermissionsInfo.BioinfoReportOperateAuditTaskReview == '1':
                temp_OldData = models.BioinfoDataAnalysisInfo.objects.filter(sam_code_num=sam_code_num,
                                                                             DataAnalysis_num=DataAnalysis_num)
                if Operate_Examine_Result == '通过':
                    if not temp_OldData[0].Operate_Examine_Result == Operate_Examine_Result and \
                                    temp_OldData[0].Report_Send_Sign == '0' or not \
                                    temp_OldData[0].Report_Send_Man == Report_Send_Man:
                        # 添加系统消息
                        Title = '通知：生信报告发送任务'  # 系统消息标题
                        Message = username + '分派给你一个生信报告发送任务！样本编号为：' + sam_code_num + '。请尽快完成任务！'  # 系统邮件正文
                        models.UserSystemMessage.objects.create(
                        ...
                        sendEmail(Report_Send_Man, Title, Message)  # 发送邮件通知
                elif Operate_Examine_Result == '不通过':
                    if not temp_OldData[0].Operate_Examine_Result == Operate_Examine_Result:
                        sample = models.BioinfoDataAnalysisInfo.objects.get(sam_code_num=sam_code_num,
                                                                            DataAnalysis_num=DataAnalysis_num)
                        user = sample.ReportMakeTask_Man
                        # 添加系统消息
...
#  运营审核报告信息详情页
def Bioinfo_Report_Operate_Audit_ShowData (request):
    try:
        username = request.session['username']
        department = request.session['department']
    except Exception:
        ...
        if button_name == 'seeInfo':
            return render(request, "modelspage/Bioinfo_Report_Operate_Audit_ShowData.html",
                          {"data": temp_mysql, "userinfo": temp, "myInfo": temp_myInfo,
                           "SystemMessage": temp_SystemMessage_Unread,
                           "num_SystemMessage_Unread": num_SystemMessage_Unread})
        elif button_name == 'ModifyData':
            temp_userlist = User.objects.filter(first_name='市场部')
            return render(request, "modelspage/Bioinfo_Report_Operate_Audit_ModifyData.html",
                          {"data": temp_mysql, "userinfo": temp, "myInfo": temp_myInfo, "userlist": temp_userlist,
                           "SystemMessage": temp_SystemMessage_Unread,
                           "num_SystemMessage_Unread": num_SystemMessage_Unread})
