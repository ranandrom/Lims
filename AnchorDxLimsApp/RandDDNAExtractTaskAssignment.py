...
# 研发样本DNA提取任务未分配详情页
def RandDSampleDNAExtractTaskInfo (request):
    try:
        ...
        # temp_userlist = models.UserInfo.objects.filter(department='软件工程部')  # 用户信息
        temp_userlist = User.objects.filter(first_name='研发部')
        # for i in range(0, len(temp_userlist)):
        #     print temp_userlist[i].username

        temp_RandDSampleInfo = models.RandDSampleInfo.objects.filter(sam_code_num=sam_code_num)
        temp_RandDSamplePretreatmentInfo = models.RandDSamplePretreatmentInfo.objects.filter(sam_code_num=sam_code_num)
        if len(temp_RandDSamplePretreatmentInfo) == 0:
            TaskProgress = '样本登记'
        else:
            TaskProgress = '样本预处理'
        return render(request, "modelspage/RandDDNAExtractTaskAssignment.html",
                      {"data": temp_RandDSampleInfo, "userinfo": temp, "myInfo": temp_myInfo,
                       "TaskProgress": TaskProgress, "RandDSamplePretreatmentInfo": temp_RandDSamplePretreatmentInfo,
                       "userlist": temp_userlist, "SystemMessage": temp_SystemMessage_Unread,
                       "num_SystemMessage_Unread": num_SystemMessage_Unread})

# 研发样本DNA提取任务分配操作
def RandDSampleDNAExtractTaskAssignmentOperation (request):
    try:
        username = request.session['username']
        department = request.session['department']
    except Exception:
...
# 研发样本DNA提取任务已分配信息详情页
def RandDSampleDNAExtractTaskDetails (request):
    try:
        username = request.session['username']
        department = request.session['department']
    except Exception:
        ...
        if button_name == 'DNA_audited':
            return render(request, "modelspage/RandDDNAExtractTaskDetails.html",
                          {"data": temp_RandDSampleInfo, "userinfo": temp, "myInfo": temp_myInfo,
            ...
            temp_userlist = User.objects.filter(first_name='研发部')
            return render(request, "modelspage/RandDDNAExtractTaskAssModifyData.html",
                          {"data": temp_RandDSampleInfo, "userinfo": temp, "myInfo": temp_myInfo,
                           "TaskProgress": TaskProgress, "userlist": temp_userlist,
                           "RandDSamplePretreatmentInfo": temp_RandDSamplePretreatmentInfo,
                           "TaskDetails": TaskDetails, "SystemMessage": temp_SystemMessage_Unread,
                           "num_SystemMessage_Unread": num_SystemMessage_Unread})