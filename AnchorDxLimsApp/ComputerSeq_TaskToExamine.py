        ...
        if temp_UserOperationPermissionsInfo.clinicalExperimentalTaskAssignment == '1':
            ...
            if button_name == 'Determine':
                ...
                if Next_TaskProgress == request.POST.get('DNA_extraction'):
                    ...
                    # 添加系统消息
                    Title = '通知：临检样本上机测序任务'  # 系统消息标题
                    Message = username + '分派给你一个临检样本上机测序任务！样本编号为：' + sam_code_num + '。请尽快完成任务！'  # 系统邮件正文
                ...
                if not Next_TaskProgress == OldTaskProgress:
                    ...
                    if Next_TaskProgress == request.POST.get('DNA_extraction'):
                        ...
                        temp_data = models.FinLibConInfo.objects.filter(sam_code_num=sam_code_num,
...
def task_To_Examine_Determine (request):
    try:
        ...
        if button_name == 'ComputerSeq_audited':
            return render(request, "modelspage/Comseq_TaskToExamineDetermine.html",
                          {"data": temp_mysql, "userinfo": temp, "myInfo": temp_myInfo,
                           "SystemMessage": temp_SystemMessage_Unread,
                           "num_SystemMessage_Unread": num_SystemMessage_Unread})
        elif button_name == 'ModifyData':
            temp_userlist = User.objects.filter(first_name='临检中心')
            return render(request, "modelspage/Comseq_ModifyAssData.html",
                          {"data": temp_mysql, "userinfo": temp, "myInfo": temp_myInfo, "userlist": temp_userlist,
                           "SystemMessage": temp_SystemMessage_Unread,
                           "num_SystemMessage_Unread": num_SystemMessage_Unread})